...
/* Created on:     02/06/2017 14:31:15                          */
/*==============================================================*/
...
create table dbo.Houses (
   HouseId              int                  identity(1, 1) not for replication,
   ProductSiteId        int                  not null,
   LocalityId           int                  null,
   HouseAttrID          int                  null,
...
alter table dbo.FuelNorms
   add constraint FK_FUELLNORMATTRS_FUELNORMID foreign key (FuelNormAttrID)
      references dbo.FuelNormAttrs (FuelNormAttrID)
go
...
alter table dbo.Houses
   add constraint FK_HOUSES_HOUSEATTRID foreign key (HouseAttrID)
      references dbo.HouseAttrs (HouseAttrID)
         on update cascade on delete cascade
go
