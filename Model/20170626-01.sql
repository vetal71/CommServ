...
/* Created on:     26/06/2017 16:06:33                          */
/*==============================================================*/
...
create table dbo.BudgetKindRef (
   BudgetKindId         smallint             not null,
...
create table dbo.LocalitiesRef (
   LocalityID           int                  identity(1, 1) not for replication,
...
create table dbo.StreetRef (
   StreetId             int                  identity(1, 1) not for replication,
...
alter table dbo.ConsumerAttrs
   add constraint FK_CONSUMERATTS_ATTRID foreign key (AttrId)
      references dbo.AttrsRef (AttrId)
...
alter table dbo.Consumers
   add constraint FK_CONSUMERS_LOCALITYID foreign key (LocalityId)
      references dbo.LocalitiesRef (LocalityID)
go
...
alter table dbo.Counters
   add constraint FK_COUNTERS_COUNTERKINDID foreign key (CounterKindId)
      references dbo.CounterKindsRef (CounterKindId)
go
...
alter table dbo.Houses
   add constraint FK_HOUSES_STREETREFID foreign key (StreetId)
      references dbo.StreetRef (StreetId)
go
...
alter table dbo.ProductSites
   add constraint FK_PRODSITES_LOCATIONTYPEID foreign key (LocationTypeId)
      references dbo.LocationTypesRef (LocationTypeId)
go

alter table dbo.TariffServs
   add constraint FK_TSERVS_SERVKINDID foreign key (ServiceKindId)
      references dbo.ServiceKindRef (ServiceKindId)
         on update cascade on delete cascade
go
